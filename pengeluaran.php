...
function getFinancialStats($db) {
    try {
        ...
        return [
            ...
            'total_transaksi_pengeluaran' => $expense_stats['total_transaksi_pengeluaran'] ?? 0
        ];
    } catch (Exception $e) {
    ...
    try {
        ...
        switch($action) {
            case 'create':
                ...
                $expense->tanggal = $_POST['tanggal'];
                $expense->deskripsi = $_POST['deskripsi'];
                $expense->kategori = $_POST['kategori'];
                $expense->jumlah = str_replace(',', '', $_POST['jumlah']); // Remove commas if any
                $expense->status = $_POST['status'];

                if($expense->create()) {
                    echo json_encode(['success' => true, 'message' => 'Pengeluaran berhasil ditambahkan!']);
                } else {
                    throw new Exception("Gagal menambahkan pengeluaran ke database");
                }
                break;
            ...
            case 'update':
                ...
                if($expense->update()) {
                    echo json_encode(['success' => true, 'message' => 'Pengeluaran berhasil diperbarui!']);
                } else {
                    throw new Exception("Gagal memperbarui pengeluaran di database");
                }
                break;
            ...
            case 'delete':
                ...
                if($expense->delete()) {
                    ...
                    throw new Exception("Gagal menghapus pengeluaran dari database");
                }
                break;
...
$limit = 10;
$total_records = $expense->getTotalCount();
$total_pages = ceil($total_records / $limit);

// Get expenses
$stmt = $expense->getAll($page, $limit);
$expenses = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Get statistics
$stats = $expense->getStats();

// Get income stats for dashboard (from transactions table)
try {
    $income_query = "SELECT SUM(CASE WHEN status = 'Selesai' THEN jumlah ELSE 0 END) as total_pendapatan FROM transactions";
    $income_stmt = $db->prepare($income_query);
    $income_stmt->execute();
    $income_stats = $income_stmt->fetch(PDO::FETCH_ASSOC);
    $total_pendapatan = $income_stats['total_pendapatan'] ?? 0;
} catch (Exception $e) {
    $total_pendapatan = 245780000; // fallback value
}

$keuntungan_bersih = $total_pendapatan - $stats['total_pengeluaran'];
?>
...
<body class="flex flex-col min-h-screen">
  ...
  <header class="headerr text-white py-8">
    ...
    <div class="container mx-auto px-8 pt-32">
      ...
      <h1 class="text-3xl font-bold mb-6 text-white">Pengeluaran Perusahaan</h1>
      
      <div class="flex justify-between items-center">
        <div class="flex space-x-4">
          <!-- Tombol Pendapatan -->
          <a href="pendapatan.php" class="border border-yellow-400 text-yellow-400 px-6 py-3 rounded font-medium flex items-center">
            Pendapatan
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
      ...
      <form id="formTambahTransaksi">
        ...
        <div class="mb-4">
          ...
          <input type="text" id="deskripsi" name="deskripsi" class="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="Masukkan deskripsi pengeluaran" required>
        </div>
        ...
        <div class="mb-4">
          ...
          <input type="number" id="jumlah" name="jumlah" class="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="Masukkan jumlah pengeluaran" required>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
      ...
      <form id="formEditTransaksi">
        ...
        <div class="mb-4">
          ...
          <input type="text" id="edit_deskripsi" name="edit_deskripsi" class="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="Masukkan deskripsi pengeluaran" required>
        </div>
        ...
        <div class="mb-4">
          ...
          <input type="number" id="edit_jumlah" name="edit_jumlah" class="w-full px-3 py-2 border border-gray-300 rounded-md" placeholder="Masukkan jumlah pengeluaran" required>
        </div>
  ...
  <div id="deleteConfirmModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
      ...
      <p class="text-gray-600 mb-6">Apakah Anda yakin ingin menghapus pengeluaran "<span id="deleteItemName"></span>"? Tindakan ini tidak dapat dibatalkan.</p>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Popup form functionality for Tambah Pengeluaran
      ...
      // Edit Pengeluaran functionality
