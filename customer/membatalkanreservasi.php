...
// Cek jika ada permintaan POST untuk membatalkan
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_to_cancel'])) {
    $reservation_id = $_POST['id_to_cancel'];
    // Ambil email dari sesi yang sudah divalidasi di header
    $user_email = $_SESSION['email']; 

    try {
        $stmt = $conn->prepare("DELETE FROM bookings WHERE id = :id AND email = :email");
        $stmt->bindParam(':id', $reservation_id, PDO::PARAM_INT);
        $stmt->bindParam(':email', $user_email, PDO::PARAM_STR);
        $stmt->execute();
        
        // Arahkan kembali untuk refresh halaman dengan status sukses
...
//Logika Mengambil Data Reservasi untuk Ditampilkan
try {
    // Ambil email dari sesi yang sudah divalidasi di header
    $user_email = $_SESSION['email'];
    $stmt = $conn->prepare("SELECT id, workspace, desk_number, start_date, start_time, end_time FROM bookings WHERE email = :email ORDER BY start_date DESC, start_time ASC");
    $stmt->bindParam(':email', $user_email, PDO::PARAM_STR);
    $stmt->execute();
    $reservations = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    // Jika gagal, siapkan array kosong dan pesan error
    $reservations = [];
  ...
  <body class="flex flex-col min-h-screen bg-gray-50">
    ...
    <main class="flex-grow">
      <div class="container mx-auto p-8">
        <!-- Notifikasi sukses setelah pembatalan -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php if (!empty($reservations)): ?>
                <?php foreach ($reservations as $row):
                    // Pemetaan gambar berdasarkan nama workspace
                    $image_map = [
                ...
                ?>
                    <div class="bg-white border rounded-lg shadow-md hover:shadow-lg transition-shadow flex flex-col">
                      ...
                      <div class="p-5 flex flex-col flex-grow">
                        <h3 class="text-lg font-semibold mb-1"><?php echo htmlspecialchars($row['workspace']); ?> Desk <?php echo htmlspecialchars($row['desk_number']); ?></h3>
                        ...
                        <!-- Form untuk membatalkan, dengan konfirmasi dari browser -->
            ...
            <?php endif; ?>
            <!-- Pesan jika terjadi error database -->
